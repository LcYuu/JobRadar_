...
export const findEmployerCompany = (currentPage, size) => async (dispatch) => {
    dispatch({ type: GET_EMPLOYER_COMPANY_REQUEST });
    try {
        const response = await api.get(`/job-post/employer-company?page=${currentPage}&size=${size}`); // Thay thế với URL thực tế
        dispatch({
        ...
        dispatch({
            type: GET_EMPLOYER_COMPANY_FAILURE,
            payload: error.message 
        });
    }
};
