...
const JobStatsPage = () => {
  ...
  useEffect(() => {
    const fetchViewStats = async () => {
      try {
        ...
        const viewStatsResponse = await axios.get(`${API_URL}/job-stats/view-stats`, {
          headers: { 'Authorization': authToken }
        });
        
        // Lấy bài đăng hiệu suất tốt nhất
        const bestJobsResponse = await axios.get(`${API_URL}/job-stats/best-performing-jobs`, {
          headers: { 'Authorization': authToken }
        });
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-purple-700 mb-8">Thống kê hiệu suất tuyển dụng</h1>
      
      {/* Card thống kê tổng quan */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="p-6 shadow-md">
          <h2 className="text-lg font-semibold text-gray-700 mb-2">Tổng lượt xem</h2>
          <p className="text-3xl font-bold text-purple-600">{viewStats.totalViews}</p>
          <p className="text-sm text-gray-500 mt-1">Tổng số lượt xem tất cả bài đăng</p>
        </Card>
        
        <Card className="p-6 shadow-md">
          <h2 className="text-lg font-semibold text-gray-700 mb-2">Tổng bài đăng</h2>
          <p className="text-3xl font-bold text-blue-600">{viewStats.totalJobs}</p>
          <p className="text-sm text-gray-500 mt-1">Số lượng bài đăng của bạn</p>
        </Card>
        
        <Card className="p-6 shadow-md">
          <h2 className="text-lg font-semibold text-gray-700 mb-2">Tổng lượt ứng tuyển</h2>
          <p className="text-3xl font-bold text-green-600">{totalApplicationCount}</p>
          <p className="text-sm text-gray-500 mt-1">Tổng số lượt ứng tuyển tất cả bài đăng</p>
        </Card>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-blue-700 mb-8">
        <h2 className="font-semibold mb-2">Làm thế nào để cải thiện hiệu suất bài đăng?</h2>
