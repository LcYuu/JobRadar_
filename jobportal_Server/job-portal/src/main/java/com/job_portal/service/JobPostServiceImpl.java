...
public JobPost updateJob(JobPostDTO jobPostDTO, UUID postId) throws AllExceptions {
	...
	if (existingJob.isEmpty()) {
		throw new AllExceptions("Không tìm thấy công việc này");
	}
	...
	if (jobPostDTO.getCityId() != null) {
		...
		if (newCity.isPresent() && !newCity.get().equals(oldJob.getCity())) {
	...
	// Nếu có bất kỳ trường nào được cập nhật, lưu vào DB
	if (isUpdated) {
		return jobPostRepository.save(oldJob);
	}
	
	// Nếu không có gì thay đổi, trả về đối tượng gốc
	return oldJob;
}
...
public List<JobPost> searchJobByJobName(String title, UUID userId) throws AllExceptions {
	try {
		...
		throw new AllExceptions("Lỗi khi tìm kiếm công việc: " + e.getMessage());
	}
}
